...
2. expansion erases ordinary calls and records active self-references as finite
   pattern edges;
3. authoritative operation steps contain only ADD, MULTIPLY, PHASE, and INDEX;
...
`PHASE(0)`, `ADD()`, `MULTIPLY()`, `PHASE(2)`, `ADD()`. Its report also has
...
The recursive-pattern example reports one `PHASE` operation and one pattern
