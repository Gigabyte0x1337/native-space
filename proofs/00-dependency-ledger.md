...
| T-LANG-CONCAT-1 | Trailing variadic packs substitute finite source expressions in order, and `concat(d, values...)` lowers visibly and exactly to position-depth INDEX terms joined by ADD with no runtime opcode | D-LANG-1, D-LANG-CONCAT-1, D-NS-2, D-NS-5, T-LANG-INTERP-1 | Proved |
| T-LANG-FOLD-1 | `fold(f,z,values...)` is exactly the finite left-nested application of one ordinary binary source function and lowers with no fold opcode | D-LANG-1, D-LANG-FOLD-1, D-LANG-2 | Proved |
| T-LANG-CAMERA-1 | `camera(a,b,v)` deterministically selects terms carrying INDEX direction `a`, remaps or unwraps that complete depth, and lowers every closed finite use to the same exact core state | D-LANG-CAMERA-1, D-NS-2, D-LANG-2, T-LANG-INTERP-1 | Proved |
| T-LANG-VECTOR-1 | `output E as vector` maps zero or one unindexed oriented scalar `(x,y)` to the exact cone coordinates `(x^2-y^2,2xy,x^2+y^2)`, which satisfy the cone identity and identify exactly opposite nonzero orientations | D-LANG-VECTOR-1, T-CONE-1 | Proved |
| T-LANG-DATA-1 | Complete-data execution passes every decoded JSON/NSBATCH root item to one selected source function in file order without host-defined model transforms | D-LANG-DATA-1, D-LANG-2, D-LANG-BATCH-1 | Proved |
| T-LANG-TRACE-1 | `trace(f)` returns an injective nested native coordinate strand for the complete finite source graph reachable from `f`, including recursive call edges | D-LANG-TRACE-1, D-NS-2, L-SEP-2, uniqueness of UTF-8, finite map/set semantics | Proved |
| T-LANG-LENGTH-1 | `length(trace(f))` moves the exact finite trace-coordinate count onto unary INDEX depth; MULTIPLY composes lengths and a positive factor witnesses strict shortening | D-LANG-TRACE-1, D-LANG-LENGTH-1, D-NS-5, D-NS-6 | Proved |
| T-LANG-UNTRACE-1 | `untrace(v,rank)` returns an exact held-out-validated recurrence when one exists in the order-at-most-32 grammar; otherwise it counts every earlier-to-later symbol pair by positive distance, preserves the exact counted symbol dictionary, and retains the deterministic ceiling of the ranked channel fraction; omitted rank is one | D-LANG-UNTRACE-1, D-LANG-TRACE-1, exact field elimination, finite bounded-order search, finite map counting, D-NS-2 | Proved |
| T-LANG-RANK-1 | Exact `rank_descent` deterministically replays finite relationship patterns, records the first exact mismatch, and selects only a tested candidate reproducing the complete data-sized rank-one reference; `apply(rank_descent(...),p)` returns the unique positive one-based replay row | D-LANG-RANK-1, T-LANG-UNTRACE-1, finite map counting, canonical native-state equality | Proved for agreement one; no global minimality or unseen-continuation claim |
| T-LANG-RANK-POLICY-1 | Static rank retains the exact ceiling of the requested channel fraction and accepts exactly when the measured finite matching-row ratio meets the exact rational threshold | D-LANG-RANK-1, exact integer ordering and cross multiplication | Proved; thresholds below one are explicitly lossy measurements, not equivalence proofs |
| T-LANG-BATCH-1 | Rectangular host arrays of rank at most 64 lower injectively by axis-as-INDEX-direction and position-as-depth; version-1 binary packing preserves state and shape; CPU execution equals ordered pointwise repeated evaluation; every accepted exact signed-32-bit scalar GPU batch equals CPU, while unsupported values and overflow are rejected | D-LANG-BATCH-1, D-NS-2, T-LANG-INTERP-1, exact in-range two's-complement arithmetic | Proved |
| E-LANG-FREQUENCY-1 | A bounded Rust host experiment projects a finite exact indexed state to classical complex `f64`, retains a deterministic projected-power mode prefix, and reports success only when replayed samples meet the caller's measured absolute-error bound | T-LANG-INTERP-1, explicit lossy classical camera, finite numerical replay verification | Implemented and tested experiment; not a native-equivalence or continuation theorem |
...
| ID | Required statement | Dependencies | Status |
|---|---|---|---|
| T-FAPP-TRACE-1 | Variadic `derive_all` preserves argument order and concatenates the exact nested operation traces | D-FAPP-1, finite sequence iteration | Proved |
...
| T-MAT-RANK1-CONTRACTION-1 | Two factor-known rank-one matrices multiply through one shared inner coefficient at every output coordinate | L-NS-5, L-NS-7, finite ADD and MULTIPLY | Proved; closed exact witness in `examples/tryouts/matrix-coefficient-contraction.ns` |
| E-MAT-COEFFICIENT-1 | Compare compiled dense multiplication with factor-known coefficient contraction at four square sizes | T-MAT-RANK1-CONTRACTION-1, `language/runtime/examples/matrix_coefficient_benchmark.rs` | Observed on one Intel Core i9-9900K; machine-specific, not a general matrix-compression result |
...
| T-NS-TRIAD-COEFFICIENT-1 | One parameterized 2D Fourier triad family contracts its two ordered interactions to one coefficient, and Leray projection plus viscosity preserve zero divergence | L-NS-5, L-NS-7, T-PERSPECTIVE-ZERO-1, finite exact dot and projection algebra | Proved; closed exact witness in `examples/tryouts/navier-stokes-fourier-triad.ns` |
| E-NS-SPH-INDEX-1 | Compare the same finite WCSPH update under all-pairs and spatial-INDEX candidate enumeration | `applications/native-water/water-model.mjs`, `applications/native-water/benchmark.mjs`, numerical equivalence tests | Observed on one Intel Core i9-9900K; 1.56x at 512 through 8.63x at 3,990 particles; structural JavaScript result, not Native runtime speed |
| E-NS-SPH-CONTRACT-1 | Retain each unordered supported SPH pair once and reuse its density, pressure, and viscosity coefficients for both directed updates | E-NS-SPH-INDEX-1, pairwise algebra in `applications/04-navier-stokes-turbulence.md`, numerical equivalence tests | Observed on one Intel Core i9-9900K; 1.36x through 1.66x beyond ordinary INDEX at 216 through 3,990 particles across two runs; handwritten JavaScript, not Native compiler speed |
| E-NS-SPH-VULKAN-1 | Evaluate one identical retained SPH coefficient graph with optimized single-threaded Rust and Vulkan compute | E-NS-SPH-CONTRACT-1, `language/runtime/examples/native_water_native.rs`, `language/runtime/examples/native_water_native.wgsl` | Observed on one i9-9900K and RTX 4090 across two 31-round runs; synchronized Vulkan was 2.92x at 3,990, 50.81x at 49,152, and 32.39x at 147,456 particles in the second run; handwritten fixed-graph kernel, not full simulation or Native compiler output |
