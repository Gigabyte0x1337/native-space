...
| T-LANG-UNTRACE-1 | Every successful `untrace(v,alpha)` result is the deterministically selected shared scalar recurrence over complete native states with the lowest exact held-out position-error ratio inside the declared order-at-most-32 grammar, reports every mismatch, obeys ratio `alpha`, and is returned as a finite recursive operation strand; complete file input is retained as one ordered in-memory sequence and omitted `alpha` means zero | D-LANG-UNTRACE-1, D-LANG-TRACE-1, exact field elimination, finite bounded-order search | Proved |
