...
| D-LANG-UNTRACE-1 / T-LANG-UNTRACE-1 | Definition / Proved | `untrace(v,alpha)` searches exact shared scalar recurrences of order at most 32 over complete native states, keeps complete file input as one ordered in-memory sequence, minimizes the exact held-out position-error ratio before program size, enforces rational `alpha` from zero through one, and reports every mismatch; omitted `alpha` means zero | Paper proof in `proofs/04-language-semantics.md`; scalar, multidimensional, over-64-observation, tolerated-error, malformed-layout, idempotence, and prime-overfit rejection tests in Rust |
