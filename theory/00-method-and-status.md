...
| D-LANG-CONCAT-1 / T-LANG-CONCAT-1 | Definition / Proved | Exact-state trailing packs spread finite source arguments, while `concat` visibly lowers them to position-depth INDEX terms joined by ADD | Paper proof in `proofs/04-language-semantics.md`; pure-source expansion, direct/VM equality, forwarding, host invocation, and located-error tests in Rust |
| D-LANG-FOLD-1 / T-LANG-FOLD-1 | Definition / Proved | A finite fold is exactly left-nested application of one ordinary binary source function and introduces no fold opcode | Paper proof in `proofs/04-language-semantics.md`; empty-pack, source-expansion, host-invocation, and zero-proof tests in Rust |
| D-LANG-CAMERA-1 / T-LANG-CAMERA-1 | Definition / Proved | The explicit indexed camera selects one present direction and remaps or unwraps its complete depth; discarded or merged coordinates remain declared losses | Paper proof in `proofs/04-language-semantics.md`; selection, remapping, unwrapping, closed lowering, and evaluator/VM tests in Rust |
| D-LANG-VECTOR-1 / T-LANG-VECTOR-1 | Definition / Proved | `as vector` exposes the exact three-coordinate quadratic cone camera for zero or one unindexed oriented scalar and rejects states it cannot represent without silent flattening | Paper proof in `proofs/04-language-semantics.md`; exact 3-4-5, zero, rejection, parser, evaluator, and VM example tests in Rust |
| D-LANG-DATA-1 / T-LANG-DATA-1 | Definition / Proved | Complete-data execution passes all decoded root items to one selected source function in file order without host-defined model semantics | Paper proof in `proofs/04-language-semantics.md`; ordered variadic invocation and source-defined transition-frequency example in Rust |
| D-LANG-TRACE-1 / T-LANG-TRACE-1 | Definition / Proved | `trace(f)` returns the complete finite reachable source graph as a nested native operation strand; recursive calls remain finite edges | Paper proof in `proofs/04-language-semantics.md`; direct/compiled and recursive trace tests in Rust |
| D-LANG-LENGTH-1 / T-LANG-LENGTH-1 | Definition / Proved | `length(S)` projects a canonical operation strand's exact coordinate count onto unary INDEX depth; MULTIPLY composes those lengths | Paper proof in `proofs/04-language-semantics.md`; exact-depth, strict-factor, malformed-strand, and evaluator/VM tests in Rust |
| D-LANG-UNTRACE-1 / T-LANG-UNTRACE-1 | Definition / Proved | `untrace(v,rank)` first requires exact held-out deterministic recurrence agreement; if none exists, it counts all ordered symbol relationships by distance, preserves the exact counted dictionary, and retains the strongest exact rank fraction; omitted rank is one | Paper proof in `proofs/04-language-semantics.md`; deterministic scalar/vector, relationship fallback, full-rank, fractional-rank, source-round-trip, invalid-rank, malformed-layout, and idempotence tests in Rust |
| D-LANG-RANK-1 / T-LANG-RANK-1 | Definition / Proved | Exact `rank_descent` generates one finite rank-one reference and selects only a tested pattern matching every row; source omission is adaptive and a supplied static target defaults to exact agreement | Paper proof in `proofs/04-language-semantics.md`; Rust tests cover exact adaptive, linear CLI, and static rejection; no global rank-minimality or unseen-continuation claim |
| T-LANG-RANK-POLICY-1 | Proved | Static rank uses exact ceiling channel selection and exact rational finite agreement; any threshold below one is labeled lossy and is not semantic equivalence | Paper proof in `proofs/04-language-semantics.md`; exact-rational boundary, ceiling, acceptance, static-selection, and report tests in Rust |
| D-LANG-BATCH-1 / T-LANG-BATCH-1 | Definition / Proved | A unary function runs sequentially inside each independent point; rectangular arrays of rank at most 64 lower injectively by axis-as-INDEX-direction and position-as-depth; version-1 binary packing preserves native state and host shape; accepted scalar GPU output equals CPU output | Paper proof in `proofs/04-language-semantics.md`; JSON/binary round-trip, multidimensional collision, CPU, live CPU/GPU agreement, and GPU-overflow tests; implementation not formally verified and no parsing or execution speedup claimed |
| E-LANG-FREQUENCY-1 | Implemented experiment | A bounded Rust host pass projects a finite exact indexed state to classical complex `f64`, selects a deterministic sparse frequency replay, and emits it only after measured finite-window error satisfies the caller's bound | Design and limits in `language/FREQUENCY.md`; quarter-turn, constant, and invalid-window tests in Rust; camera-relative and finite only, with no native-equivalence, continuation, or speedup claim |
...
| T-MAT-RANK1-CONTRACTION-1 | Proved | Two factor-known rank-one matrices use one shared inner multiplication coefficient for every output coordinate | General finite proof and closed exact witness in `../applications/01-matrix-and-llm-compression.md` |
| E-MAT-COEFFICIENT-1 | Observed | Compiled factor-known coefficient contraction used less CPU time than ordinary dense multiplication at sizes 64 through 512 | Two seven-round local runs and reproducible harness recorded in `../applications/01-matrix-and-llm-compression.md`; no factor-discovery or arbitrary-matrix claim |
| T-NS-TRIAD-COEFFICIENT-1 | Proved | One finite 2D Fourier-triad family shares one multiplication coefficient, and its pressure-projected nonlinear-plus-viscous derivative remains divergence-free | General family proof and closed exact witness in `../applications/04-navier-stokes-turbulence.md`; no full solver or regularity claim |
| E-NS-SPH-INDEX-1 | Observed | The same finite WCSPH coefficient update runs with all-pairs or 3D spatial-INDEX candidate enumeration | One five-round local benchmark measured 1.56x at 512 through 8.63x at 3,990 particles with maximum state disagreement at most 1.1e-9; manually compiled JavaScript, not Native runtime speed |
| E-NS-SPH-CONTRACT-1 | Observed | One unordered supported pair retains shared geometry plus density, pressure, and viscosity gains and applies the equivalent two directed updates | Two consecutive five-round local runs measured 1.36x through 1.66x beyond ordinary INDEX at 216 through 3,990 particles; maximum observed state disagreement 1.3e-9; handwritten JavaScript, not Native compiler speed |
| E-NS-SPH-VULKAN-1 | Observed | One identical retained SPH coefficient graph runs through optimized single-threaded Rust and Vulkan compute | Two 31-round local runs on an i9-9900K and RTX 4090; second-run synchronized Vulkan speedups were 2.92x, 50.81x, and 32.39x at 3,990, 49,152, and 147,456 particles; fixed graph and handwritten Rust/WGSL only |
| H-MAT-1 / H-LLM-1 / H-DYN-1 / H-NBODY-1 / H-NS-1 / H-PROT-1 / H-PL-1 | Hypothesis | Falsifiable application expectations | Defined in `../applications`; most remain unexecuted and the observed matrix/SPH subcases do not establish the broad hypotheses |
| E-MAT-1 / E-LLM-1 / E-DYN-1 / E-NBODY-1 / E-NS-1 / E-PROT-1 / E-PL-1 | Experiment | Planned experiment or implementation procedures corresponding to those hypotheses | Broad procedures remain planned; observed coefficient/SPH subexperiments are registered separately |
